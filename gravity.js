  ...
  check_and_init_new_elements();
            ...
            function initContactListener() {
                ...
                contactListener.BeginContact = function(contact) {
                    ...
                    playCollisionSound(); // Play sound on collision
                };
            ...
            function playCollisionSound() {
                var collisionSound = new Audio('./drop.mp3'); // Preload the audio
                collisionSound.play(); 
...
function check_and_init_new_elements() {
  // Get box2d elements
  const current_elements = getElementsByClass("box2d");
  ...
  for (var i = 0; i < current_elements.length; i++) {
    ...
    if (!is_initialized) {
      ...
      element.style.left = prop[0] + "px";
      element.style.top = prop[1] + "px";
      element.style.width = prop[2] + "px";
      ...
      let isWall = element.classList.contains("wall");
...
// 作っていた物理演算要素を全部消す。
function destroy_all_box2d_bodies() {
...
function loop() {
  ...
  // 新しい文字が打ち込まれていたら物理演算子を初期化
  check_and_init_new_elements();

  delta[0] += (0 - delta[0]) * 0.5;
  delta[1] += (0 - delta[1]) * 0.5;
  
  // ここで、スマホの加速度を取得して、重力に適用する。
  // gravity.x = ...
  // gravity.y = ...
